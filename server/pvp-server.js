...
handlers['room:leave'] = (ws) => { const uid=requireAuth(ws); if(!uid)return; const r=findRoomByUser(uid); if(!r){send(ws,{type:'error',error:'not in a room'});return;} removeUserFromRoom(r.id,uid); };
...
handlers['room:start'] = (ws, msg) => {
  ...
  if (room.players.size<2) { send(ws,{type:'error',error:'need 2+ players'}); return; }
