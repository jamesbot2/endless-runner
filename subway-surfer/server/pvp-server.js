...
handlers['room:leave'] = (ws) => { const uid=requireAuth(ws); if(!uid)return; const r=findRoomByUser(uid); if(!r){send(ws,{type:'error',error:'not in a room'});return;} send(ws, {type:'room:left',roomId:r.id}); const rid=r.id; removeUserFromRoom(rid,uid); };
...
handlers['room:start'] = (ws, msg) => {
  ...
  if (room.players.size<1) { send(ws,{type:'error',error:'need at least 1 player'}); return; }
  for (const [,p] of room.players) if (!p.ready) { send(ws,{type:'error',error:'not all ready'}); return; }
